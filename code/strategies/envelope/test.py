...
if strategy == 'scalping':
    ...
    params = {
        ...
        'leverage': 2,
        'grid_distance': 5,         # USD between grids
        'num_grids': 5,
        'stop_loss': 5,             # USD
        'trail_stop_activate_grid': 2,  # Activate trailing from grid 2
        'trailing_stop_distance': 6,    # USD
        'trend_filter': True,
        'trend_ema_period': 50,
        'max_active_grids': 4,
    }
...
class GridTrader:
    ...
    def update_stop_management(self):
        if self.position:
            ...
            if self.position['side'] == 'long':
                grid_levels = [entry_price + i * params['grid_distance'] 
                               for i in range(1, params['num_grids'] + 1)]
            else:
                grid_levels = [entry_price - i * params['grid_distance'] 
                               for i in range(1, params['num_grids'] + 1)]
    ...
    def check_stop_conditions(self):
        if self.trailing_stop and self.position:
            current_price = float(self.position['markPrice'])
            if current_price <= self.trailing_stop['stop_price']:
                self.close_position()
        ...
        while True:
            for symbol, engine in engines.items():
                try:
                    ...
                    # Fetch trend data if trend filter is enabled
                    if params.get('use_trend_filter'):
